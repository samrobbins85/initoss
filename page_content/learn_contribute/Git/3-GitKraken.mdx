...
![GitKraken Main View](/img/learn_contribute/git/gitkraken/GitKraken_Main.webp)

This looks complicated, so we'll walk through each section

## Top bar

![GitKraken Top Bar](/img/learn_contribute/git/gitkraken/top.png)

Many of the commands here are just replicas of git commands, so **Pull** is
`git pull` etc. As discussed before, `git push` needs to be accompanied with the
...
![GitKraken Right sidebar](/img/learn_contribute/git/gitkraken/right_bar.png)
...
![GitKraken Left sidebar](/img/learn_contribute/git/gitkraken/left_bar.png)

In general you will use this sidebar much less than the right sidebar, but it is
...
![GitKraken Left sidebar](/img/learn_contribute/git/gitkraken/middle.png)
...
At the very top you can see `//WIP`, this is to show that I have uncommited
changes.
